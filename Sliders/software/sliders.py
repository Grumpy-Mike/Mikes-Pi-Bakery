# Pi Christmas Sliders Game 40 setups
...
def waitLevel():  # hold until the platform is level
   table = getSensor()
   while table != 15 :
      firstLook = getSensor()
      time.sleep(1.0)
      secondLook = getSensor()
      if firstLook == secondLook :
         table = firstLook

def getMove(sensor):
   global move,moveInc,moveIncX,moveIncY,piece
   move = 0
   if sensor == 0xd: # up
      move = 1
      moveInc = -5
...
def drawLevelScreen():
   ...
   drawWords("Set platform level",cp-100,cp-100)
