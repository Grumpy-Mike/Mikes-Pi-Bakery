...
instruments = ["Marimba","Synth","Piano","Bells"] #just add folder names here nothing else needed
instrumentPointer = 0
freePlay = True
   ...
   while True:
      ...
      if freePlay :
         for pin in range (0,len(pinList)):
           if io.input(pinList[pin]) == 0:
              #print pin
              samples[pin].play()
              time.sleep(0.090)
              while io.input(pinList[pin]) == 0:
                  pass
...
def initGPIO():
   try :
      ...
      os._exit(1)
   for pin in range (0,len(pinList)):
      io.setup(pinList[pin],io.IN, pull_up_down=io.PUD_UP) # make pin into an input
