...
class PicoTrillLib:
    ...
    def __init__(self, bus, tt, addr, sclp, sdap, speed): # tt - trill type string addr - I2C address
        self.dataBuffer = [0] * kRawLength
        self.rawData = [0] * kNumChannelsMax
        self.bus = bus
        self.i2c = I2C(bus, scl=Pin(sclp), sda=Pin(sdap), freq=speed) # selects from choice of pins on the Pico
