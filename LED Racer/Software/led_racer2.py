#!/usr/bin/env python3
# LED Racer2 - by Mike Cook June 2019
# with speed limits
...
lastPos = [0,0] ; spinOff = [False,False]
spinOffTime = [time.time(),time.time()]
...
def main():
  global position,lap,screenUpdate, done, winner, speed, lastPos
...
def updatePosition():
  global position, screenUpdate, lap, lastPos, done
  global winner, speed, spinOff, spinOffTime
  for i in range(0,2):
    #print(i,path[lastPos[i]])
    #print("speed",speedLimit[path[lastPos[i]]])
    if speed[i] > speedLimit[path[lastPos[i]]]:
      #print("spin off car",i)
      skid.play()
      spinOff[i] = True
      spinOffTime[i] = time.time() + 2.5 # 2.5 time penalty
      screenUpdate = True    
    if not spinOff[i] :  
      ...
      if time.time() > spinOffTime[i]: # restore car
        spinOff[i] = False
        speed[i] = 0
        screenUpdate = True
...
def updateLEDs(pos0,pos1):
    ...
    if(trackLEDs):
       drawSpeed()
       if pos0 == pos1 and spinOff[0] == False and spinOff[1] == False:
...
def drawSpeed(): # draw speed limit & all wipe others
   global pixels
...
def drawScreen(pos0,pos1):   
   ...
   for i in range(0,num_pixels):
     if speedLimit[i] < 90 :
       screen.blit(speedRec,(led_pointX[i],led_pointY[i]))
   ...
   drawWords("Speed Limit "+str(int(limit * 100))+" on red",360,470,black,backCol)
   if spinOff[0]:
     drawWords("Spin off",35,450,black,backCol)
   else:
     drawWords("Speed",35,450,black,backCol)
     drawWords(str(int(speed[0]*100)), 87,450,black,backCol)
   if spinOff[1]:
     drawWords("Spin off",630,450,black,backCol)
   else:  
   ...
   if pos0 == pos1 and not spinOff[0] and not spinOff[1]:
...
def init():
    global track, flag, led_pointX, led_pointY, path, spi, speedChange
    global speedLimit, whistle, end, speedRec, skid, limit
    whistle = pygame.mixer.Sound("sounds/whistle.ogg")
    end = pygame.mixer.Sound("sounds/end.ogg")
    skid = pygame.mixer.Sound("sounds/skid.ogg")
    ...
    speedLimit = [99.0] * len(path) # speed limit start off with 99
    limit = 0.10
    for i in range(64,68):
      speedLimit[i] = limit # max speed on bend
    ...
    speedRec = pygame.Surface((10,10)) #transparent speed limit
    speedRec.set_alpha(96)
    speedRec.fill((255,0,0))
...
def initLEDs():
    global pixels, trackLEDs, num_pixels
    trackLEDs = True
    pixel_pin = board.D18
    num_pixels = 184
    ORDER = neopixel.GRB
