# Raspberry Pi - Switching the number Danse Macabre
# Using the spectrum display hardware from MagPi 46
# By Mike Cook - September 2016
...
def main():
   print"Danse Macabre skeleton initilising"
   initGPIO()
   loadImages()
   initWindow()
   xbase1 =((xs/4)-108); xbase2 =(3*(xs/4)-18);xbase0 =((xs/2)-78)
   print"now click on iconified Danse tab"
   print"the space bar controls full screen display"
   print"keys 1, 2 & 3 for the number of skeletons"
   foreArmMax = 36; ArmMax = 36   
   thighMax = 40; legMax = 36   
   moveThresh = 0.08
   while True:
      getSpectrumData()
      pan = int((xs/32)*(leftData[0]-rightData[0]))
      ...
      if numberOfSk == 2 or numberOfSk == 3:
         showPicture(ArmPosL,ArmPosR,foreArmPosL,foreArmPosR,thighPosL,thighMax-thighPosR-1,legPosL,legPosR,pan+xbase2,tilt)
         showPicture(ArmPosL,ArmPosR,foreArmPosL,foreArmPosR,thighPosL,thighMax-thighPosR-1,legPosL,legPosR,pan+xbase1,tilt)
      if numberOfSk == 1 or numberOfSk == 3:
         showPicture(ArmPosL,ArmPosR,foreArmPosL,foreArmPosR,thighPosL,thighMax-thighPosR-1,legPosL,legPosR,pan+xbase0,tilt)
