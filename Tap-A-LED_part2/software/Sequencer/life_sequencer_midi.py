#!/usr/bin/env python3
# Life Sequencer_MIDI by Mike Cook October 2020
# after each frame the game of life rules are applied to the display 
...
def main():
    global markTime, stepTime, lifeCells
    init()
    setTime()
    drawScreen()
        ...
        if tap.touched() and not beingTouched:
            ...
            if pos[3] : # a valid reading
                if pixels[pos[2]] != [0, 0, 0]:
                   ...
                   life = posToxy(pos[2])
                   lifeCells[life[0]][life[1]] = False
                else:
                   pixels[pos[2]] = colours[pos[1]]
                   life = posToxy(pos[2])
                   lifeCells[life[0]][life[1]] = True                   
                beingTouched = True   
                pixels.show()
        else :
...
def init():
    ...
    global markerRect, incRect, midiout
    global decRect, icon, decRect, muteRect, voiceRect, sampleRect
    global font, rsRect, tempo, lastNote, lastChan, chan, note
    global velocity, mute, update, lineCol, backCol, hiCol
    global running, stepTime, step, samples
    global lifeCells, lifeWorking
    ...
    pygame.display.set_caption("Game of Life Sequencer")
    ...
    step = 0 ; samples = 16
    initMIDI()
    lifeWorking = [[False for x in range(8)] for x in range(16)]
    lifeCells = [[False for x in range(8)] for x in range(16)]
...
def putCol(pos): # restors old coloum of colours
    for i in range(8):
...
def doLife():
    global lifeWorking, lifeCells, pixels
    for i in range(8) :
        for k in range(16):
            count = countSurround(k, i)
            if lifeCells[k][i] : #if cell "alive"
                if count < 2 or count > 3 :
                    lifeWorking[k][i] = False # kill it
                else :
                    lifeWorking[k][i] = True # let it remain
            else : # cell dead
                if count == 3 : lifeWorking[k][i] = True    
    # restore display
    for i in range(8) :
        for k in range(16):
            pp = i + (k * 8)
            if lifeWorking[k][i] :
                ...
                pixels[pp] = (0, 0, 0)
    pixels.show()
    lifeCells = copy.deepcopy(lifeWorking)

def countSurround(xT, yT) :
    c = 0
    incX = [-1, 0,  1, -1, 1, -1, 0, 1]
    incY = [-1, -1, -1, 0, 0, 1, 1, 1]
    for i in range(8) :
        x = xT + incX[i]
        x &= 0x0F
        y = yT + incY[i]
        y &= 0x07
        if lifeCells[x][y] : c += 1 # count "living" cells
    return c    
